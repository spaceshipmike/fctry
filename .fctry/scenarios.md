...
**Satisfied when:** The user can discover inspiration from pluggable knowledge sources without knowing or typing URLs. Gap detection uses a three-layer model: Layer 1 (structural, zero LLM cost) identifies thin sections, low scenario coverage, never-evolved content, low experience-language density, missing interaction state coverage, and absent experience references automatically; Layer 2 (quality signals) surfaces sections with repeated build failures, Observer absence findings, constraint erosion, visual/interaction quality issues, and UX-related verification debt; Layer 3 (developer assessment) is the human's strategic judgment applied at session time. Source discovery is stack-aware — the system reads the project's tech-stack from the spec frontmatter and searches the relevant package registry (npm, cargo, pypi), filters GitHub results by stack, and flags cross-stack pattern transfer when inspiration comes from a different ecosystem. Source modularity works: without knowmarks, the system searches GitHub and inbox items; without gh CLI, it uses knowmarks and inbox. No single source is required. Auto-query mode (`/fctry:ref discover`) generates searches from project state. Single-source mode (`/fctry:ref knowmarks "query"`) searches that specific source. Batch selection researches items in parallel. Novelty filtering prevents re-incorporating patterns the spec already addresses. When a source is unavailable, the system says so clearly and continues with remaining sources — the user never sees a cryptic MCP error.

Validates: `#ref-flow` (2.5), `#capabilities` (3.1)

#### Scenario: Design-Aware Gap Detection and Discovery

> **Given** A spec section describes a UI feature using mostly implementation-oriented language ("the system renders a list," "items are stored") with no mention of interaction states (hover, loading, empty, error), no linked experience references, and no UX vocabulary (transitions, feedback, animation)
> **When** The discovery loop runs gap detection on this section
> **Then** The system identifies it as a design gap — not just a functional gap — and generates design-oriented search queries (interaction patterns, component design systems, UX case studies) rather than functional code references, so the discovered inspiration strengthens how the feature looks and feels, not just what it does

**Satisfied when:** Gap detection distinguishes between functional weakness (thin text, low scenario coverage) and design weakness (low experience-language density, missing interaction states, no experience references). When a section has design-weighted gap signals, source discovery searches for design inspiration — interaction pattern libraries, UX references, visual design systems — rather than defaulting to functional code references. The same section can have both functional and design gaps, and the discovery pipeline generates queries for both. Observer verification assesses visual/interaction quality (feedback, transitions, state completeness, visual consistency) alongside functional correctness when chunks target sections with UI components, classifying design findings separately from functional findings.
