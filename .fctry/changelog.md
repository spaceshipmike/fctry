...
- `#capabilities` (3.1) cross-session learnings: Updated lesson-to-instruction graduation from "future direction" to concrete ratchet-gated mechanism referencing the instruction optimization loop [modified]

## 2026-03-15T12:00:00Z — /fctry:evolve capabilities (hard/soft ratchet modalities)
- Spec version: 3.85 → 3.86 (1 section modified, 0 added, 0 removed)
- `#capabilities` (3.1): Split quality ratchet into two evaluation modalities — hard ratchet (deterministic, external: scenario evaluator subprocess, ast-grep structural metrics, file sizes, performance benchmarks — crisp numbers, automatic revert on regression) and soft ratchet (probabilistic, LLM-judged: design quality, spec compliance, readability — high-confidence signal to investigate, not automatic revert). The distinction reflects that autoresearch's single-metric crispness works because the entire problem fits in context; fctry's chunks can exceed context, so LLM-judged dimensions need appropriately lower gatekeeping force [modified]

## 2026-03-15T11:00:00Z — /fctry:ref capabilities+execute-flow+experience-references (evaluation-gated commits / quality ratchet)
- Spec version: 3.83 → 3.85 (3 sections modified, 0 added, 0 removed)
- `#capabilities` (3.1): Added evaluation-gated commits (quality ratchet) pattern — changes only land if they provably improve measured outcomes across six dimensions: scenario satisfaction, performance, token economy, code modularity, design quality, and gap detection scores. The Observer captures a quality baseline before each chunk and compares after; regressions trigger revert-and-restructure instead of commit. Monotonic within a build. Also updated experimental autonomous loop verify step to use the quality ratchet [modified]
- `#execute-flow` (2.7): Added per-attempt retry budget (fixed ceiling per retry attempt, making attempts comparable and preventing context domination) and structured retry journal (per-attempt results logged to build trace with what-was-tried, Observer verdict, metric deltas, and keep/discard/crash status — analogous to autoresearch's results.tsv) [modified]
- `#experience-references` (5.2): Added karpathy/autoresearch — autonomous overnight experimentation with evaluation-gated commits, simplicity criterion, fixed-budget trials, and indefinite autonomous operation. Informed fctry's quality ratchet pattern across all loops [modified]
