...
Add a project dashboard as the viewer's landing page — cards for every registered project showing aggregated state (readiness, build status, inbox count, untracked changes) and a recommended next command. The dashboard becomes the primary observation and decision-making surface: the user looks and decides here, Claude Code executes. Add inbox consumption to `/fctry:evolve` and `/fctry:ref` so queued ideas flow into the conversation as context. The CLI remains the execution engine; the viewer becomes the place where the user understands the state of all their projects at a glance.
