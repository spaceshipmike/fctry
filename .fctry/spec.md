...
**Context-aware execution.** The Executor treats the context window as a finite resource, ensuring that context pressure never degrades build quality. Each chunk operates with sufficient context to do its work well, regardless of how many chunks preceded it. The system structures work to create natural context boundaries — checkpoint state persists through files (`.fctry/state.json`, git commits, CLAUDE.md) rather than conversation history, so the build remains sharp across any number of chunks. When the Executor detects that a build requires unusual context management — a chunk too large for one context window, or a dependency chain that would accumulate excessive state — it calls this out in the build plan. Otherwise, context management is invisible: it's just how the system works. The compact instructions in CLAUDE.md guide what's preserved if auto-compaction occurs mid-build, and the build checkpoint ensures that even a full context clear results in a resumable build, not a lost one.
...
- **Main content** — The spec rendered as a clean, readable document. Each section header includes a subtle copy button — clicking it copies that section's Markdown to the system clipboard for pasting into Slack, email, or another conversation.
...
| Spec ambiguity discovered during build | "The spec says the list is sorted by urgency, but doesn't describe how urgency is determined for items without a due date. Should those items appear (1) at the top (most urgent), or (2) at the bottom (least urgent)?" | Answer the experience question; execution resumes |
...
**Live spec viewer with project dashboard.** The system serves specs as a local web UI via a single multi-project-aware server. The viewer's landing page is a dashboard showing all registered projects as cards with aggregated state — readiness summary, build status, inbox count, untracked changes, and a recommended next command as a copyable chip. The user scans the dashboard to decide which project needs attention and what to do next, then executes in Claude Code. Clicking a project card drills into the spec view. Within the spec view, the user can switch between all registered fctry projects from a sidebar without opening separate browser tabs. The viewer updates specs in real-time via WebSocket as agents work, highlights the section currently being edited, and displays change history with diffs. The viewer is read-only and requires no build step.
...
**Token economy output rules.** Extends the concise output rule with specific constraints on how agents handle evidence and repetition:

- **Reference-first evidence.** When citing evidence (log entries, file contents, command outputs), agents include a reference ID and a short note — never the raw content. The viewer hydrates references into full excerpts for display. This prevents agents from pasting large blocks of context into their output.
...
- **codebase-digest** (https://github.com/kamilstanuch/codebase-digest) — An AI-friendly codebase packer with 60+ structured analysis prompts. Its token counting as a project intelligence metric informed fctry's codebase size estimation, and its section-level clipboard copy pattern informed the viewer's copy button.
...
**Then:** Viewer as decision surface.

Add a project dashboard as the viewer's landing page — cards for every registered project showing aggregated state (readiness, build status, inbox count, untracked changes) and a recommended next command as a copyable chip. The dashboard becomes the primary observation and decision-making surface: the user looks and decides here, Claude Code executes. Add inbox consumption to `/fctry:evolve` and `/fctry:ref` so queued ideas flow into the conversation as context. The CLI remains the execution engine; the viewer becomes the place where the user understands the state of all their projects at a glance.
...
- **Section readiness distribution.** What fraction of sections are in each readiness state (draft, needs-spec-update, spec-ahead, aligned, ready-to-execute, satisfied, deferred)? A project with many `needs-spec-update` sections has code outpacing the spec. A project with many `spec-ahead` sections has a detailed spec but little implementation.
...
| **Section readiness** | Per-section metadata indicating the section's current state: `draft`, `needs-spec-update`, `spec-ahead`, `aligned`, `ready-to-execute`, `satisfied`, or `deferred`. Assessed by the State Owner during every scan and updated by the Executor after each build chunk. Stored authoritatively in `state.json` as a `sectionReadiness` map; all display surfaces (status line, viewer, dashboard) read from this single source. `deferred` marks intentionally-postponed sections — counted as "ready" in aggregation. |
